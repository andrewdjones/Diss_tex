...
\begin{figure}
	...
	\label{critiques}
	\includegraphics[width=6.25in]{critiques.png}
\end{figure}
...
\begin{figure}
	...
	\label{k_t}
	\includegraphics[width=6.25in]{kockelman_3simple.png}
\end{figure}



%Loran, his way
$S_1$ = outlines of shapes on 2D painting surface
$I_1$ = ``carry-through" lines of volumetric tension
$O_1$ = formal configurations with 2D indices and 3D implications
%Loran, Wollheim's way
$S_1$ = lines and colors on 2D painting surface
$O_1$ = psycho-spatial(?) objects or schema with 2D indices and embodied or enminded implications
$I_1 = S_2$ = identification of shapes on 2D painting surface
$O_2$ = 2D-represented objects with 3D volumes
$I_2$ = carry-through lines of volumetric tension
%So: BOTH processes require objects with different properties than those captured by or interpretable from the indices of the painting

%Strunk
$S_1$ = lead sheet chords
$I_1$ = tonal-prolongational ``carry-through" lines of syntactic function
$O_1$ = tonal configurations with chord surface indices and implications for long-range hearing or Schenkerian coherence
%in particular, he turns patterns of the appearance of EM (and chords interpretable relative to EM) into a IV prolongational span
%Strunk via Waters
$S_1$ = lead sheet chords
$O_1$ = pre-conceived, embodied/enminded tonal span objects with a variety of surface indices
$I_1 = S_2$ = potential prolongational segmentations
$O_2$ = categories of common tonal-prolongational behavior
$I_2$ = syntax disambiguating ``tonal and harmonic ambiguity"
%the properties of $O_1$ that permit the sense-making of prolongational interpretants $I_1$ necessarily exceed indices of surface.  Some kind of hearing-as occurs that can't be demonstrated from the indices, and it seems to involve an appeal to expected norms to disambiguate.
%Waters
$S_1$ = lead sheet chords
$O_1$ = schema with signed surface indices
$I_1 = S_2$ = potential schema segmentations
$O_2$ = categories of syntax-preserving substitutable temporal progressions
$I_2$ = syntax disambiguating chord sequence

%Terefenko (as I read him)
$S_1$ = pitches at (quantized) times
$O_1$ = tonal, rooted, roughly-tertian pitch class sets
$I_1 = S_2$ = Roman Numerals
$O_2$ = categories of syntax-preserving substitutable roman numeral ``functions"
$I_2 = S_3$ = possible functions
$O_3$ = well-formed functional contexts
$I_3$ = disambiguated function
%My project replaces the first two of these, but provides no guidance for the third?
...
%the choice of what this syntax (these interpretants) serve(s) as indices for -- what it will communicate with -- depends on the type of interpretant the analyst wants and the nature of the objects involved in that particular semotic ontology

%well-formedness
%if deployed on a homogeneous corpus, can assess various types of probabilistic model
%if deployed on a heterogenous corpus, can assess the (non)normativity of a phrase
